  ...
  <section class="home-page">
    ...
    <div class="submit-panel">
      <button
        ...
        @click="handleSubmit"
      >
        {{ isSubmitting ? "正在生成中..." : "生成方案" }}
      </button>
      <div class="submit-panel__status">
        当前已接上 `/trip/generate`，生成成功后会直接展示真实旅行方案。
      </div>
    </div>
  </section>
</template>
...
.section-title {
  ...
  margin-bottom: 18px;
  padding-bottom: 14px;
  border-bottom: 1px solid rgba(33, 150, 120, 0.14);
  color: #244a57;
  font-size: 16px;
  font-weight: 700;
}
...
.pill-box {
  ...
  min-height: 32px;
  border-radius: 12px;
  background: linear-gradient(135deg, #20b2aa 0%, #4f8ee8 100%);
  color: #ffffff;
  font-weight: 700;
}
...
.submit-panel__button {
  ...
  border-radius: 999px;
  padding: 14px 28px;
  background: linear-gradient(135deg, #0fa3a2 0%, #3f86ea 100%);
  color: #ffffff;
  font-size: 15px;
  font-weight: 700;
  cursor: pointer;
  box-shadow: 0 18px 35px rgba(47, 137, 189, 0.24);
}
