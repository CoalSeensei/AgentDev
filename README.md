...
智能旅行系统是一个面向中文旅行场景的 AI 行程规划项目。用户输入目的地、日期、预算、人数和偏好后，系统会生成结构化旅行方案，并补充地图点位、天气信息、预算拆分、景点图片与可导出的旅行文档。

## 开发进程

- `2026-04-15`：加入 Redis 缓存，覆盖天气、地图和 RAG 检索结果。
- `2026-04-25`：完成 RAG 在线优化，加入 Query Rewrite 和轻量 Rerank。
- `2026-04-29`：扩充攻略知识库和评估样例，减少跨目的地污染。
...
![行程生成结果页](./assets/showcase/result-overview.png)

### 地图与天气

![地图与天气](./assets/showcase/result-map.png)

### 历史列表

![历史列表](./assets/showcase/history.png)
...
- 🌦️ **天气感知提示**：前端展示天气预报，并根据雨天或阴天自动修正旅行提示
- ⚡ **缓存与导出**：覆盖天气、地图与 RAG 缓存，并支持 Markdown 和中文 PDF 导出
